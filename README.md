...
- **プロット強化**: IEDFピーク自動検出とΔE表示（Riley見積もりとの比較付き）、
  シース厚さの振動波形（1D: 電子フロント s_e(t) とChild瞬時幅 / 2D: ウェハ・
  リングのChild瞬時幅。電位波形CSVにも列を追加）、
...
- **静磁場（オプション）**: 一様静磁場 (Bx, By, Bz) をイオンにのみ作用させる
  （Borisプッシャー、全成分0で無効=従来と同一経路）。フォームに座標系と
  B方向を示す向き図を表示。有効時は検証表に ω_ci·Δt とジャイロ半径/シース幅比を追加
...
静磁場オプションは**イオンにのみ**ローレンツ力を作用させる（Boris法）。
電場の与え方（KCL回路・Child則・Laplace基底）は無磁化電子を仮定したままのため、
電子の磁化がシース構造や密度分布そのものを変える領域
（マグネトロン級の強磁場、面平行BでのE×B閉じ込めが支配的な条件）は適用外。
座標系はフォームの向き図とモデル解説ページを参照。
